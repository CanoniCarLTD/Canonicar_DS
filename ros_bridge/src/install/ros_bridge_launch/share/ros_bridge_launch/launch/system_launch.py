...
def generate_launch_description():
    ...
    host_arg = DeclareLaunchArgument(
        "host", default_value="localhost", description="Host address for CARLA server"
    )

    port_arg = DeclareLaunchArgument(
        "port", default_value="2000", description="Port number for CARLA server"
    )
    ...
    track_line_file = os.path.join(
        get_package_share_directory("map_loader"), "map_loader", "track.line"
    )

    track_xodr_file = os.path.join(
        get_package_share_directory("map_loader"), "map_loader", "generatedTrack.xodr"
    )
    ...
    delayed_spawn_vehicle = TimerAction(period=5.0, actions=[spawn_vehicle_launch])
    ...
    # Vehicle Manager Node
    vehicle_manager_node = Node(
        package="vehicle_manager",  # Replace with the package that has VehicleManagerNode
        executable="vehicle_manager_node",  # The Python node for managing the vehicle
    ...
    ld.add_action(vehicle_manager_node)
